...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔍 ᴊᴏɪɴ ᴀɴᴅ ꜱᴇᴀʀᴄʜ ʜᴇʀᴇ 🔎", url=f"https://t.me/Movie_studio_request_group")]]))
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href='https://t.me/Movie_studio_request_group'>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/Movie_studio_request_group")]]))
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("🔍 ꜱᴇᴀʀᴄʜ ʜᴇʀᴇ 🔎", url="https://t.me/Movie_studio_request_group")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            lazy_stream = f"{URL}watch/{log_msg.id}?hash={get_hash(log_msg)}"
            lazy_download = f"{URL}{log_msg.id}?hash={get_hash(log_msg)}"                  
            buttons = [[
                    InlineKeyboardButton("🚀 ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ", url=lazy_download),
                    InlineKeyboardButton("ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ 🧿", url=lazy_stream)
                ],[
                    InlineKeyboardButton('🧿 Wᴀᴛᴄʜ ᴏɴ ᴛᴇʟᴇɢʀᴀᴍ 🖥', web_app=WebAppInfo(url=lazy_stream))#Don not remove credit else gay @Silicon_Bot_Update 
                ]]            
    ...
    # don't change anything without contacting me @creatorrio
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⏳️ Sᴛᴀᴛᴜꜱ', callback_data='status')
        ],[
        ...
        if query.from_user.id not in ADMINS:
            return await query.answer("𝚃𝙷𝙸𝚂 𝙸𝚂 𝙽𝙾𝚃 𝙵𝙾𝚁 𝚈𝙾𝚄 𝙾𝙽𝙻𝚈 𝙰𝙳𝙼𝙸𝙽𝚂 😎", show_alert=True)        
        buttons = [[
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='help')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons = [[
            InlineKeyboardButton('ᴜsᴇʀs ❤️‍🔥', callback_data='users'),
            InlineKeyboardButton('ᴀᴅᴍɪɴ ⚡', callback_data='admic')
        ],[
        ...
        buttons = [[
            InlineKeyboardButton('ᴏᴡɴᴇʀ', user_id=int(1667104725)),
            InlineKeyboardButton('ꜱᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url='https://t.me/Movie_Studio_Request')
        ], [
            InlineKeyboardButton('🏴‍☠️ ʜᴏᴍᴇ', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_sticker("CAACAgUAAxkBAAECFtdmsDdhBJiufIp0SMj6DUZlxO3TVgACShMAApBYEVVkRM2CHzHedh4E",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f'Seaching for {search} 🔎', url=f"https://t.me/Moviestudioabhi")]]) 
            )
            search = search.lower()
            find = search.split(" ")
            search = ""
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap+="<blockquote>🍿 𝒀𝑶𝑼𝑹 𝑭𝑰𝑳𝑬𝑺 ☟</blockquote>\n\n</b>"
            for file in files:
                cap += f"<b>\n<a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'> 📗 ❗️ {get_size(file.file_size)}❗️ {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>"
    else:
        if settings["button"]:
            cap = f"<b>📢 Rᴇꜱᴜʟᴛꜱ Fᴏʀ ☞  <code>{search}</code>\n🔎 Rᴇꜱᴜʟᴛꜱ Fᴏᴜɴᴅ ☞ <code>{total_results}</code>\n🧑‍💻 Rᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞{message.from_user.mention}\n\n<blockquote>⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️</blockquote>/n/n</b>"
        else:
            cap = f"<b>📢 Rᴇꜱᴜʟᴛꜱ Fᴏʀ ☞  <code>{search}</code>\n🔎 Rᴇꜱᴜʟᴛꜱ Fᴏᴜɴᴅ ☞ <code>{total_results}</code>\n🧑‍💻 Rᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞{message.from_user.mention}\n\n<blockquote>⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️</blockquote>/n<blockquote>🍿 𝒀𝑶𝑼𝑹 𝑭𝑰𝑳𝑬𝑺 ☟</blockquote>/n/n</b>"
            # cap+="<b>Hᴇʏ {message.from_user.mention}, Hᴇʀᴇ ɪs ᴛʜᴇ ʀᴇsᴜʟᴛ ғᴏʀ ʏᴏᴜʀ ᴏ̨ᴜᴇʀʏ {search} \n\n</b>"
            for file in files:
                cap += f"<b><a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'> 📗 ❗️ {get_size(file.file_size)}❗️ {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b>"
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        if not g_s:
            ...
            button = [[
                       InlineKeyboardButton("📝 ʀᴇǫᴜᴇꜱᴛ ʜᴇʀᴇ", url=f"https://t.me/Movie_Studio_Request")
            ]]
            if NO_RESULTS_MSG:
